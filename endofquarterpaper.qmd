...
All our tests were run with presence/absence data, in order to keep the data from being affected by outliers like Vogelherd. We ran a Mantel test between an Euclidean abundance distance matrix and a jaccard distance matrix of the sign types in order to make sure that the exclusion of abundance would not significantly impact our results [@Lycett_2019], and found that it was not quite statistically significant (P = `r p_value_abundance_correlation`, Mantel R = `r r_value_abundance_correlation`). 
Another Mantel test was run between a euclidean distance matrix for MedianBP and the jaccard sign type matrix in order to determine whether there was a definitive correlation between distance in time and sign types. The Mantel test found that there was a statistically significant correlation (p = `r chron_p`, Mantel-R = `r chron_r`).
