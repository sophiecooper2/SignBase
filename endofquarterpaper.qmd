...
sign_material_df <- signbase_full_clean %>% 
  group_by(material) %>% 
  summarise(count = n())%>% 
  arrange(desc(count))

iv_count <- sign_material_df %>% 
  filter(material == "ivory") %>% 
  pull(count)

bone_count <-sign_material_df %>% 
  filter(material == "bone") %>% 
  pull(count)

lime_count <- sign_material_df %>% 
  filter(material == "limestone") %>% 
  pull(count)

ant_count <- sign_material_df %>% 
  filter(material == "antler") %>% 
  pull(count)

tooth_count <- sign_material_df %>% 
  filter(material == "animal tooth") %>% 
  pull(count)

ochre_count <- sign_material_df %>% 
  filter(material == "ochre") %>% 
  pull(count)

undet_count <- sign_material_df %>% 
  filter(material == "undetermined") %>% 
  pull(count)


stone_row <- tibble("material" = c("stone"), "count" = c(1))

sign_material_df <- sign_material_df %>% 
  rbind(stone_row) %>% 
  filter(material != "stone indet.")

misc_string <- sign_material_df %>% 
  filter(count == 1) %>% 
  dplyr::select(material) %>% 
  c()

misc_string <- misc_string$material

object_type_df <- signbase_full_clean %>% 
...
The modularity score quantifies the strength of the groupings, showing that the variations in grouping strength between phases are minimal. The Proto-Aurignacian exhibits the strongest groupings, with a score of `r proto_modularity`, and the Evolved Aurignacian has the weakest groupings, with a score of `r evolved_modularity`. There is an increase in modularity between the Transitional period and the Proto-Aurignacian, before an decrease into the Early Aurignacian, followed by an increase with the Evolved Aurignacian.
...
The geographic boundaries of the groups change from phase to phase (@fig-group-map). In the Transitional phase, sites are widely spread out across Western Europe, and tend to be located closely to the other sites in their groups. The exception is Hohle Fels, which, instead of being located further west with the rest of the sites in the broader group, is located further east, closer to the sites in the restricted group, and almost overlapping with Geissenklösterle. In the Proto-Aurignacian, we see a slightly narrower distribution of sites westward but a spread northward, and more overlapping of sites between groups. In the Early Aurignacian, we see a reduced distribution, with sites clustered closer together. In the Evolved Aurignacian, we see a wider distribution and more distance between sites.

One factor that may influence the geographic structure of sites is the possibility that some sites functioned as aggregation sites. The aggregation/dispersion pattern is defined by @conkley_agg as a type of hunter-gather settlement pattern, in which groups both break off in dispersed fragments and congregate in large groups, typically for various subsistence or social reasons. Aggregation sites are the places where those fragments or individuals congregate, and some archaeological indicators of aggregation sites include proximity to certain environmental resources, as well as a more diverse artifact assemblage [@conkley_agg]. Previous studies have also identified evidence of aggregation/dispersion patterns among the early modern humans of the Upper Paleolithic [@Maher_Conkey_2019; @bahn1982inter; @Bourdier_2013; @Svoboda_2013; @bahn1982inter; @white1992rethinking; @Soffer_Adovasio_Enloe_Audouze_Zubrow_2010; @montet-white1994]. It is therefore possible that the sites with multiple connections to both groups, such as La Ferrassie in the the Proto-Aurignacian, Grotte de la Verpillière I in the Early Aurignroacian, and Gargas in the Evolved Aurignacian acted as aggregation sites or served a similar purpose. However, without examining environmental data, this remains only a hypothesis at this time.
...
These results are consistent with existing data on the environmental and population change established during these phases. @Shao_Wegener_Klein_Schmidt_Weniger_2024 tells us that from 45 - 43.25 k BP, the population density in Europe was extremely low, due to the unfavorable environmental conditions of Greenland Stadial 12 (44 k BP) and the slow westward expansion of early modern humans, before being followed by rapid expansion from 43.25 - 41 k BP, with the maximum extent of Proto/Early Aurignacian (Aur-P1) settlement being reached around 41 k BP. This rapid expansion could be what stimulated the emergence of signs as a marker of group membership. Other studies have also found a similar associations in other regions and time period between increases in quantity and diversity of art, and increases in population density [@smith1992Design; @hays1993symbol; @McDonald_Veth_Lilley_2006; @Lourandos_1985; @barton_clark_cohen1994; @mcdonald2012social; @bernardini2005reconsidering; @wiessner1984reconsidering; @smith1992]. 

Additionally, @Banks_dErrico_Zilhao_2013 use eco-cultural niche modeling to show that the Heinrich Stadial 4 at the very beginning of the Early Aurignacian resulted in an expansion into new environmental niches, which they hypothesize was associated with expansion of social networks both within and between populations. This likely explains why we see a large increase in both inter-connectivity and intra-connectivity strength in the Early Aurignacian, as well as the increased correlation with geographic distance. Lastly, @Shao_Limberg_Klein_Wegener_Schmidt_Weniger_Hense_Rostami_2021 developed a human-existence probability model for the Aurignacian, finding an expanded, more dispersed settlement area for Evolved/Late Aurignacian (Aur-P2) theorizing that the environmental changes of HS4 lead to humans better adapted to survive in a broader range of climate conditions. @Shao_Wegener_Klein_Schmidt_Weniger_2024 also finds an increase in expansion marking the beginning of the Evolved Aurignacian. Geographic expansion resulting in less group interconnection could explain why we see weaker, more dispersed connections in the Evolved Aurignacian.

The results also add to our knowledge of social networks within the Aurignacian period, suggesting dynamic patterns of social exchange and connection. Previous works [@bahn1982inter; @Horiuchi_Takakura_2019; @Rogers_2013; @baker2024evidence; @Golovanova_Doronichev_Doronicheva_Sapega_Shackley_2021; @Gamble_1982] have used material evidence analysis to reconstruct Upper Paleolithic social networks, which research has pointed to as a potential driver of cultural and evolutionary growth [@Greenbaum_Friesem_Hovers_Feldman_Kolodny_2019; @Creanza_Kolodny_Feldman_2017; @Cullen_1996]. @Foley_Gamble_2009 place the emergence of inter-group social networks at around 30 k BP, however, our network analysis shows evidence of inter-group social networks beginning at around 41 k BP, suggesting an earlier emergence than previously predicted. Dividing the data into smaller time periods potentially serves to rectify the low temporal resolution that tends to affect network analysis results when analyzing older, more time-averaged data sets [@Gravel-Miguel_Coward_2023; @Daems_Coco_Gillreath-Brown_Kafetzaki_2024]. Additionally, by focusing on change over time, we can see social networks as fluid and dynamic, rather than static and bounded [@Maher_Conkey_2019]

One possible limitation is the potential ambiguity that arises with some of the sign types. While @Von_Petzinger_2009 argues in favor of Aurignacian geometric signs as symbolic expression, potential ambiguity still remains over whether function is consistent across all sign type occurrences, particularly in regards to the simpler sign types composed from fewer elements. We included the more ambiguous sign types because similar sign occurrence may still serve as evidence of cultural connectivity regardless of function, and additionally, greater quantities of included sign types serves to establish stronger connections [@Gravel-Miguel_Coward_2023]. Potential ambiguity may be addressed in the future through the usage of the data recently released in SignBase 2.0 covering sites in Germany, which contains information on the specific order and patterns in which signs occurred.


# Conclusion

This study both adds to our understanding of Aurignacian social dynamics and suggests potential new directions for future research. The observed social dynamics produced by our methods align with the existing data on the environmental and population conditions of the Upper Paleolithic. We feel that our research opens up new opportunities for the application of these methods to data on artwork variation in other regions and time periods. Our research also suggests an earlier emergence of dynamic inter-connectivity than previously predicted, as we see weaker inter-connections beginning to emerge by 41 k BP, and stronger inter-connections by 38 k BP.  Future research directions might involve the analysis of environmental conditions and their interactions with social dynamics, in order to determine whether some of the sites that we have pointed to as potential aggregation sites function as such. Another potential future research direction would be the usage of the more specific data released in SignBase 2.0 in order to counter potential sign type ambiguity. 
