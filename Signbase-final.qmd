...
disim <- 1 - dm
disim[disim<0.5] <- 0
disim_net<- network(disim,directed=F,ignore.eval=F,names.eval='weight')
...
ggraph(disim_net, layout="fr") +
  ...
  geom_node_text(aes(label=row.names(artifact_data)), size=6, family= "Times New Roman", repel=T) +
...
artifact_matrix_j_dist <- as.matrix(vegdist(reordered_matrix, method = "jaccard", binary = TRUE))
