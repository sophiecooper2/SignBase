...
net_fr <- ggraph(net, layout="fr") +
  ...
  geom_node_text(aes(label=rownames(artifact_data)), size=6, family= "Times New Roman", repel=T) +
  theme(text= element_text(size = 20), panel.background = element_rect(fill = 'white'))

net_coord <- ggraph(net, layout="manual", x=signbase_unique_groups$longitude, y=signbase_unique_groups$latitude) +
  geom_edge_link(aes(width = weight, alpha=weight), edge_colour = "black", show.legend=T) + 
  scale_edge_width(range=c(0.5,1.5)) +
  scale_edge_colour_gradient(low = "#CCCCCC",high = "#000000") +    
  geom_node_point(alpha=1, size=5) + 
  geom_node_text(aes(label=rownames(artifact_data)), family= "Times New Roman", size=6, repel=T)+
  theme(text= element_text(size = 20), panel.background = element_rect(fill = 'white'))
